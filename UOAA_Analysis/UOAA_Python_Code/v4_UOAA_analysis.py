...
running code on mac
/usr/bin/python3 \
"/Users/jaime/Desktop/UOAA/v4_UOAA_analysis.py" \
  --input "/Users/jaime/Desktop/UOAA/uoaa_hollister_cleaned.json" \
  --output "/Users/jaime/Desktop/UOAA/uoaa_hollister_results.json" \
  --batch-size 200 \
  --workers 4
...
def get_input_brands(record: Dict[str, Any]) -> List[str]:
    """
    Return the brand(s) exactly as provided in the input.
    Accepts string or list; anything else -> [].
    No scanning or inference.
    """
    b = record.get("brand")
    if isinstance(b, str):
        return [b]
    if isinstance(b, list):
        return [x for x in b if isinstance(x, str)]
    return []

# ---------- NEW: merge duplicates so multi-brand posts become arrays ----------

def merge_brand_duplicates(posts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    If the same post appears multiple times with different 'brand' values,
    merge them into a single record whose 'brand' field is a list of all
    unique brands mentioned.

    We use (topic_url, post_url, user, date, text) as the dedup key, pulling
    from either top-level fields OR nested post_meta (if present).
    """

    def make_key(rec: Dict[str, Any]):
        post_meta = rec.get("post_meta") or {}
        topic_url = rec.get("topic_url") or post_meta.get("topic_url")
        post_url = rec.get("post_url") or post_meta.get("post_url")
        user = rec.get("user") or post_meta.get("user")
        date = rec.get("date") or post_meta.get("date")
        text = rec.get("text")
        return (topic_url, post_url, user, date, text)
    ...
    for rec in posts:
        ...
        if key not in grouped:
            # shallow copy so we don't mutate the original list
            new_rec = dict(rec)
            brands = get_input_brands(new_rec)
            if brands:
                new_rec["brand"] = sorted(set(brands))
            grouped[key] = new_rec
        else:
            existing = grouped[key]
            existing_brands = set(get_input_brands(existing))
            new_brands = set(get_input_brands(rec))
            all_brands = existing_brands.union(new_brands)
            if all_brands:
                existing["brand"] = sorted(all_brands)
...
def load_uoaa_posts(json_path: str) -> List[Dict[str, Any]]:
    ...
    ALSO:
    - Merges duplicate posts that only differ by 'brand' so that
      multiple brands for the same post become an array in 'brand'.
    """
        ...
        try:
            ...
            for line in f:
                try:
                    ...
                    continue
            return merge_brand_duplicates(posts)

    # Case 1: list of posts
...
"targets": {{
  "brands_mentioned": string[],          // DO NOT infer; set EXACTLY to BRAND_FROM_INPUT
...
- Use "unknown" when confidence < 90% for categorical fields other than "overall_sentiment" (e.g., primary_emotion, explicit_product_terms, pros_aspects, cons_aspects).
- Do not invent missing fields; leave arrays empty if not found.
- **Set "targets.brands_mentioned" EXACTLY to BRAND_FROM_INPUT (no additions, no removals, no normalization).**
...
BRAND_FROM_INPUT (verbatim list): {brand_from_input}
...
def analyze_post(record: Dict[str, Any],
    ...
    # 2) Use BRAND FROM INPUT ONLY (no scanning / inference)
    brands_from_input = get_input_brands(record)  # verbatim, string or list

    # 3) Build prompt with frozen brand(s)
    prompt = (
        SYSTEM_PREAMBLE
        + "\n\n"
        + INSTRUCTION_TEMPLATE.format(
            ...
            brand_from_input=json.dumps(brands_from_input, ensure_ascii=False),
            text=text
        )
    )
    ...
    # ✅ Enforce brands EXACTLY from input (no scanning)
    if isinstance(gjson, dict):
        tgt = gjson.get("targets")
        if isinstance(tgt, dict):
            tgt["brands_mentioned"] = brands_from_input[:]  # verbatim copy
        else:
            gjson["targets"] = {
                "brands_mentioned": brands_from_input[:],
                "explicit_product_terms": []
            }

    # 4) Resolved overall comes **only** from Gemini
    try:
